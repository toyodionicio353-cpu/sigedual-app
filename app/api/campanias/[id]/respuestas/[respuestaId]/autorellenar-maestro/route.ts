...
import type { EstadoMaestroGuia, Rol } from "@/types";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; respuestaId: string }> }) {
  ...
  try {
    ...
    await setDocument(`autorizaciones/${datosCampania.profesorUid}_maestro_${maestroGuiaId}`, {
    ...
    await registrarEventoServidor({
      ...
      recursoId: maestroGuiaId,
      resultado: "permitido",
      detalle: `Autorrellenado desde la campaña ${id} (respuesta ${respuestaId})${body.maestroGuiaIdExistente ? " (actualizó un maestro guía existente)" : " (creó un maestro guía nuevo)"}.`,
    });
