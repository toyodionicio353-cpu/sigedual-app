...
export async function GET(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    const campania = await getDocument(`campanias_invitacion/${token}`);
    if (!campania) {
    ...
    if (estado === "activa" && datos.expiraEn && new Date(datos.expiraEn) < new Date()) {
      estado = "expirada";
      await updateDocumentFields(`campanias_invitacion/${token}`, { estado: "expirada" });
    }
    ...
    await updateDocumentFields(`campanias_invitacion/${token}`, { ultimoAccesoEn: new Date().toISOString() });

    const respuesta: RespuestaPublica = { liceoNombre: "SIGEDUAL", profesorNombre: datos.profesorNombre };
