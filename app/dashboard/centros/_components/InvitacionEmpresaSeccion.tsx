...
/** Bloque de "Agregar centro" para invitar a una empresa a completar su
 * propio registro. Dos modalidades: "Individual" (enlace de un solo uso,
 * comportamiento original sin cambios) y "Masivo" (un mismo enlace recibe
 * muchas respuestas independientes hasta un cupo) — en ambas, cada
 * respuesta sigue siendo un solo envío con Centro + Maestro Guía juntos. */
export default function InvitacionEmpresaSeccion() {
  ...
  const [capacidadCampania, setCapacidadCampania] = useState("20");
  ...
  async function cargarCampanias() {
    ...
    let snap;
    if (usuario.rol === "profesor") {
      snap = await getDocs(query(collection(db, "campanias_invitacion"), where("profesorUid", "==", usuario.uid)));
    } else if (modoGlobal) {
      snap = await getDocs(collection(db, "campanias_invitacion"));
    } else {
      snap = await getDocs(query(collection(db, "campanias_invitacion"), where("liceoId", "==", usuario.liceoId)));
    }
    const lista = snap.docs.map((d) => ({ id: d.id, ...d.data() } as CampaniaInvitacionEmpresa));
    lista.sort((a, b) => (b.creadoEn ?? "").localeCompare(a.creadoEn ?? ""));
    setCampanias(lista);
  ...
  async function generarCampania() {
    ...
    try {
      const id = crypto.randomUUID();
      const ahora = new Date();
      const nueva: Omit<CampaniaInvitacionEmpresa, "id"> = {
        liceoId: usuario.liceoId,
        profesorUid: usuario.uid,
        profesorNombre: usuario.nombre,
      ...
      await setDoc(doc(db, "campanias_invitacion", id), nueva);
      setEnlaceCampaniaGenerado(`${window.location.origin}/invitacion/campania/${id}`);
      setCampanias((lista) => [{ id, ...nueva }, ...lista]);
      setNombreCampania("");
      setCapacidadCampania("20");
      setExpiraCampania("");
    } finally {
  ...
  async function revocarCampania(id: string) {
    ...
    setRevocando(id);
    try {
      await updateDoc(doc(db, "campanias_invitacion", id), { estado: "revocada" });
    ...
    <div style={{ borderTop: "1px solid var(--border)" }} className="mt-8 pt-8">
      <div className="mb-4">
        <h2 style={{ color: "var(--text-primary)" }} className="text-lg font-bold flex items-center gap-2">
          ...
          ¿Prefieres que la empresa complete sus datos?
        </h2>
      </div>
      ...
      {modo === "individual" ? (
        <>
          <p style={{ color: "var(--text-secondary)" }} className="text-sm mb-4">
            Genera un enlace de un solo uso para que el Centro Dual complete su información y la de sus Maestros Guía, sin necesitar una cuenta. Después de enviar el formulario, el enlace queda inutilizado. Es válido por {HORAS_EXPIRACION} horas.
          </p>
          ...
          >
            <Send size={15} />
            {generando ? "Generando..." : "Generar formulario para Empresa Dual"}
          </button>
                  ...
                  <div key={inv.id} style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }} className="rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {(inv.estado === "generado" || inv.estado === "abierto") && (
                        <>
                          <button onClick={() => copiarEnlace(`${window.location.origin}/invitacion/${inv.id}`, inv.id)} style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", color: "var(--text-secondary)" }} className="px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5">
                      ...
                      {inv.estado === "procesado" && (
                        <Link href={`/dashboard/centros/invitaciones/${inv.id}`} style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", color: "var(--text-secondary)" }} className="px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5">
        ...
        <>
          <p style={{ color: "var(--text-secondary)" }} className="text-sm mb-4">
            Genera un enlace para recibir múltiples formularios independientes hasta alcanzar el límite configurado. Cada empresa completa un solo formulario con sus dos partes (Centro/Empresa y Maestro Guía) y genera una respuesta propia — nunca comparten un mismo registro.
          </p>
          ...
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }} className="rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <div className="sm:col-span-1">
              ...
              <input value={nombreCampania} onChange={(e) => setNombreCampania(e.target.value)} placeholder="Registro Centros Duales 2027" style={inputStyle} className={inputClass} />
            </div>
            <div>
                ...
                {campanias.map((c) => (
                  <div key={c.id} style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }} className="rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {c.estado === "activa" && (
                        <>
                          <button onClick={() => copiarEnlace(`${window.location.origin}/invitacion/campania/${c.id}`, c.id)} style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", color: "var(--text-secondary)" }} className="px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5">
                      ...
                      {c.respuestasCount > 0 && (
                        <Link href={`/dashboard/centros/campanias/${c.id}`} style={{ background: "var(--accent)", color: "var(--text-on-accent)" }} className="px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5">
