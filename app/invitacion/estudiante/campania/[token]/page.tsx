...
export default function FormularioCampaniaEstudiantePage() {
  const { token } = useParams<{ token: string }>();

  const [cargando, setCargando] = useState(true);
  const [errorCarga, setErrorCarga] = useState<{ mensaje: string; tono: "danger" | "warning" } | null>(null);
  const [contexto, setContexto] = useState<ContextoCampania | null>(null);
  ...
  useEffect(() => {
    ...
    async function cargar() {
      setCargando(true);
      try {
        const res = await fetch(`/api/campanias-estudiante/${token}`);
        const data = await res.json();
        if (!res.ok) {
          setErrorCarga({ mensaje: data.error ?? "No fue posible cargar el formulario.", tono: res.status === 410 || res.status === 409 ? "warning" : "danger" });
          return;
        }
        setContexto(data);
      } catch {
  ...
  async function enviarFormulario() {
    ...
    try {
      ...
      const res = await fetch(`/api/campanias-estudiante/${token}/enviar`, {
        method: "POST",
  ...
  if (cargando) {
    return <Pantalla icon={<Clock size={26} />} titulo="Cargando formulario..." descripcion="Un momento, por favor." />;
  }
  ...
  if (errorCarga) {
    return (
      <Pantalla
        ...
        titulo={errorCarga.tono === "warning" ? "Formulario no disponible" : "Enlace no válido"}
          ...
          {paso === 6 && (
            <div>
              ...
              <p style={{ color: "var(--text-secondary)" }} className="text-sm mb-4">
                Revisa que la información sea correcta antes de enviar. Una vez enviado el formulario, no podrás modificarlo.
              </p>
